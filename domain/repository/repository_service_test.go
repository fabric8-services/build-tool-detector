...
var _ = Describe("GitServiceType", func() {
	...
	Context("CreateService", func() {
		It("Faulty Host - empty", func() {
			serviceType, err := repository.CreateService("", nil, *configuration, token)
		...
		It("Faulty Host - non-existent", func() {
			serviceType, err := repository.CreateService("test/test", nil, *configuration, token)
			Expect(serviceType).Should(BeNil(), "service type should be 'nil'")
			Expect(err.Error()).Should(BeEquivalentTo(github.ErrInvalidPath.Error()), "service type should be '400'")
		})

		It("Faulty Host - not github.com", func() {
			serviceType, err := repository.CreateService("http://test.com/test/test", nil, *configuration, token)
			Expect(serviceType).Should(BeNil(), "service type should be 'nil'")
		...
		It("Faulty url - no repository", func() {
			serviceType, err := repository.CreateService("http://github.com/test", nil, *configuration, token)
			Expect(serviceType).Should(BeNil(), "service type should be 'nil'")
		...
		It("Correct url - non-existent", func() {
			serviceType, err := repository.CreateService("http://github.com/test/test", nil, *configuration, token)
